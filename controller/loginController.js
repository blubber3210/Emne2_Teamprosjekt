...
function login() {
    ...
    const user = model.data.users.find(// Søker etter en bruker i databasen (model.data.users) som matcher brukernavn og passord
        u => u.userName.toLowerCase() === usernameInput && u.password === passwordInput
    );
    const LEmessageEl = document.getElementById('loginErrorMessage'); // Henter HTML-elementet der feilmeldinger skal vises
